  ...
  function setupChannel() {
    ...
    if (relayChannel === 'dpm' || relayChannel === 'wpm') {
      window.addEventListener('message', function(packet) {
        var host = gadgets.acceptedRemoteDomain;
        var receiver = packet.domain || packet.origin || false;
        if (receiver && host.toLowerCase() === receiver.substring(receiver.substring(0,receiver.lastIndexOf('.')).lastIndexOf('.') + 1).toLowerCase() || host.substring(host.lastIndexOf('.')).length > 4){
            process(JSON.parse(packet.data));
        } else {
